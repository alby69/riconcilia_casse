...
- [x] **Manuale utente e help integrato**: creato `docs/MANUALE_UTENTE.md` (guida semplice, in italiano, per l'operatore non tecnico) e integrato lo stesso contenuto come help richiamabile (pulsante `?` nella navbar → modal "Manuale Utente") sia in `app/cashrec.html` sia in `templates/index.html`. Il markdown resta l'unica fonte: `tools/generate_help.py` lo converte in HTML e lo inietta in entrambe le pagine (marcatori `HELP_MANUAL_START`/`END`).
